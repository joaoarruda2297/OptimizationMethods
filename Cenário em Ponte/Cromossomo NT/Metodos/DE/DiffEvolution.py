...
def main(index, componentes,num_tipos_componentes, individuos, peso_max, custo_max, num_geracoes, num_variaveis, num_max_componentes_subsistema, num_min_componentes_subsistema):
    ...
    gerador_graficos.gera_grafico(f'SolutionEvolutionDETempo{index}.png', tempos_melhor_solucao, solucoes, valor_final, geracao, 'Evolução do Tempo de Execução ao Longo das Gerações (DE)', 'Tempo (s)', 'Função Objetivo', 'Alcançado no tempo: ' + str(alg.truncate(melhor_tempo, 4)) + 's')
