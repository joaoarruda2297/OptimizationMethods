...
def main():
    ...
    for i, individuos in enumerate(populacoes):
        ...
        gerador_graficos.gera_grafico_comparativo(f'./comparativeMethodsTempo{index}.png', dicionario_metodos_tempo, 'Comparação dos Algoritmos por Tempo até a Melhor Solução', 'Tempo até a Melhor Solução (s)', 'log(Função Objetivo)', show_plot=False)
        ...
        for metodo, dados in dicionario_metodos_tempo.items():
            ...
            if pontos_filtrados:
                if(dados['x'][-1] != 1.0):
                    pontos_filtrados.append((1.0, dados['y'][-1]))
